...
class Fmeasure(nn.Module):
    ...
    def forward(self, input, gt):
        ...
        y[gt> 0] = 1
        y[gt< 0] = 0         
        tp =   (x * y).sum(dim=[1,2,3])
        tn = ((1-x) * (1-y)).sum(dim=[1,2,3])
        fp = ((1 - y) * x).sum(dim=[1,2,3])
        fn = (y * (1 - x)).sum(dim=[1,2,3])

        precision = tp / (tp + fp + 1e-7)
        recall = tp / (tp + fn + 1e-7)

        f1 = 2* (precision*recall) / (precision + recall + 1e-7)
        f1 = f1.clamp(min=1e-7, max= 1)
        return 1 - f1.mean()
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        self.attn_gated1 = AttentionGated(ngf * 2,ngf*2,int(ngf/2))
    ...
    def forward(self, input):
        ...
        up2 = self.up2(resnet+attn2)
        attn1 = self.attn_gated1(up2,down1)
        up1 = self.up1(up2+attn1)
...
if __name__ == '__main__':
    ...
    model = ResnetGenerator(1, 1 ,64, norm_layer, False, n_blocks=6)
