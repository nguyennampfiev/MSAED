...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        self.attn_gated2 = AttentionGated(ngf * 4, ngf*4, int(ngf/2))
    ...
    def forward(self, input):
        ...
        down1 = self.down1(padding) #128*128*256
        down2 = self.down2(down1) #256x64x128
        resnet = self.resblock(down2) # 256x64x128
        attn2, psi2 = self.attn_gated2(resnet,down2) # 256x64x128, 64x128
        up2 = self.up2(torch.cat((resnet,attn2),dim=1)) # 256x128x256

        attn1, psi1 = self.attn_gated1(up2,down1) #128x128x256,128x256
        input1 = self.soft(torch.cat((up2,attn1),dim=1)) #256x128x256

        up1 = self.up1(input1) # 128x256x512
        ### up1 256--> 128
        output = self.unpadding(up1)
        final_attn = self.acc_attn(torch.cat((self.upsample(psi2),psi1),dim=1))
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x):
        """Forward function (with bam connections)"""
...
class L1Weight(nn.Module):
    def __init__(self):
        super(L1Weight, self).__init__()

    def forward(self, w, fake, real):
        """
        :param real: real_img
        :param fake: fake_img
        :param weight:
        ...
        out1 = torch.abs(w*fake - real)

        loss = out1.mean() + 0.1*w.mean()
        return loss
