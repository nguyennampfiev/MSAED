...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        up2 = [nn.ConvTranspose2d(ngf * mult, int(ngf * mult/2) ,
                  ...
                  nn.ReLU(True)]
        mult = 2 ** 1
        up1 = [nn.ConvTranspose2d(ngf * mult, int(ngf * mult/2),
                                     kernel_size=3, stride=2,
                                     padding=1, output_padding=1,
                                     bias=use_bias),
                  norm_layer(int(ngf * mult/2)),
        ...
        unpadding = [nn.ReflectionPad2d(3),nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0),nn.Tanh()]
        self.attn_gated1 = AttentionGated(ngf * 2, ngf,int(ngf/4))
        self.attn_gated2 = AttentionGated(ngf * 4, ngf*2, int(ngf/2))
        ...
        self.down_attn2 =nn.Sequential(nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1, bias=use_bias),
                      norm_layer(256),
                      nn.ReLU(True))
        self.down_attn1 =nn.Sequential(nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, bias=use_bias),
                      norm_layer(128),
                      nn.ReLU(True))
    def forward(self, input):
        """Standard forward"""
        padding = self.padding(input) # 64x256x512
        down1 = self.down1(padding) # 128x128x256
        down2 = self.down2(down1) # 256x64x128
        resnet = self.resblock(down2) #256x64x128
        attn2 = self.attn_gated2(resnet,down1) #128x128x256
        down_attn2 = self.down_attn2(attn2) #256x64x128
        up2 = self.up2(resnet+down_attn2)#256x128x256 #128x128x256 
        #### up2 64*4 down1 64*2 --> 384 
        attn1 = self.attn_gated1(up2,padding)#64x256x512 
        down_attn1 = self.down_attn1(attn1) # 128x128x256
        up1 = self.up1(up2+down_attn1)
        ### up1 256--> 128
...
class AttentionGated(nn.Module):
    def __init__(self,F_g,F_x,F_int):
        ...
        self.W_x = nn.Sequential(
            nn.Conv2d(F_x, F_int, kernel_size=3,stride=2,padding=1,bias=True),
            nn.BatchNorm2d(F_int)
        )
